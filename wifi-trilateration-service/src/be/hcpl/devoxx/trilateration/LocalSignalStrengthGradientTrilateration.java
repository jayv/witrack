...
import Jama.Matrix;
import be.hcpl.devoxx.MultiTouchDrawable;
import be.hcpl.devoxx.ToolBox;
import be.hcpl.devoxx.model.AccessPoint;
import be.hcpl.devoxx.model.PointF;
import be.hcpl.devoxx.model.ProjectSite;

/**
 * trilateration impl for wifi signal strength
 * 
 * @author hanscappelle only rem gui rel code, created dummy MultiTouchDrawable
 * 
 */
public class LocalSignalStrengthGradientTrilateration extends AccessPointTrilateration {
	...
	public LocalSignalStrengthGradientTrilateration(final ProjectSite projectSite) {
		super(projectSite);
	}

	@Override
	public PointF calculateAccessPointPosition(final AccessPoint ap) {

		Vector<MeasurementDataSet> originalData = measurementData.get(ap);
		...
		if (originalData.size() > 3) {
			...
			for (MeasurementDataSet dataSet : originalData) {
			...
			for (MeasurementDataSet currentDataSet : data) {
				Vector<MeasurementDataSet> dataSetsWithinWindow = getMeasurementDataWithinWindow(data, currentDataSet,
						windowSize);

				// Make sure we can fit a plane, so we need at least 3 points.
				// And make sure the measurement points are not collinear
				if (dataSetsWithinWindow.size() >= 3 && !areAllMeasurementsCollinear(dataSetsWithinWindow)) {
					...
					for (int i = 0; i < dataSetsWithinWindow.size(); i++) {
						MeasurementDataSet dataSet = dataSetsWithinWindow.get(i);
					...
					GradientArrow arrow = new GradientArrow(currentDataSet.x, currentDataSet.y, (float) matrixC.get(0,
							0), (float) matrixC.get(1, 0), weight);
					arrows.add(arrow);
				}
			}
			...
			float minX = roundToGridSpacing(MeasurementDataSet.getMinimumValue(data, MeasurementDataSet.VALUE_X));
			float maxX = roundToGridSpacing(MeasurementDataSet.getMaximumValue(data, MeasurementDataSet.VALUE_X));
			float minY = roundToGridSpacing(MeasurementDataSet.getMinimumValue(data, MeasurementDataSet.VALUE_Y));
			float maxY = roundToGridSpacing(MeasurementDataSet.getMaximumValue(data, MeasurementDataSet.VALUE_Y));
			...
			for (float x = areaMinX; x <= areaMaxX; x += step) {
				for (float y = areaMinY; y <= areaMaxY; y += step) {
					for (GradientArrow arrow : arrows) {
						...
						float pointAngle = ToolBox.normalizeAngle((float) Math.atan2(y - arrow.y, x - arrow.x));

						float angleDifference = Math.abs(arrowAngle - pointAngle);
						...
						sumP += (float) Math.pow(angleDifference, 2) * arrow.weight;
					}
			...
			for (MeasurementDataSet probability : probabilities) {
				...
				if (probabilities.indexOf(probability) == 0 || probability.getRssi() < minProb) {
	 ...
	 */
	protected Vector<MeasurementDataSet> getMeasurementDataWithinWindow(final Vector<MeasurementDataSet> data,
			final MeasurementDataSet dataSet, final float windowSize) {
		...
		for (MeasurementDataSet currentDataSet : data) {
			...
			if (currentDataSet.getX() >= dataSet.getX() - windowSize * MultiTouchDrawable.getGridSpacingX()
					&& currentDataSet.getX() <= dataSet.getX() + windowSize * MultiTouchDrawable.getGridSpacingX()
					&& currentDataSet.getY() >= dataSet.getY() - windowSize * MultiTouchDrawable.getGridSpacingY()
					&& currentDataSet.getY() <= dataSet.getY() + windowSize * MultiTouchDrawable.getGridSpacingY()) {
	 ...
	 */
	protected boolean areAllMeasurementsCollinear(final Vector<MeasurementDataSet> measurements) {

		if (measurements.size() < 3) {
			return true;
		} else {
			PointF p1 = measurements.get(0).getPoint();
			PointF p2 = measurements.get(1).getPoint();

			for (int i = 2; i < measurements.size(); i++) {
				if (!arePointsCollinear(p1, p2, measurements.get(i).getPoint())) {
					return false;
				}
			}
	 ...
	 */
	protected boolean arePointsCollinear(final PointF p1, final PointF p2, final PointF p3) {
	...
	protected float roundToGridSpacing(final float value) {
		return Math.round(value / MultiTouchDrawable.getGridSpacingX()) * MultiTouchDrawable.getGridSpacingX();
	}
	...
	protected class GradientArrow {
		...
		public GradientArrow(final float x, final float y, final float directionX, final float directionY,
				final float weight) {
			this.x = x;
			this.y = y;
